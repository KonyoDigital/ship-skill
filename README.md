...
`verify_package.py` gates the download on **82 checks** — the shape Desktop rejects, the
...
built from. `--self-test` mutates the package and this README **58 ways** and proves the
...
python3 verify_package.py --build      # rebuild .skill/.zip from skill/
```
